...
namespace GAS.Logic
{
    ...
    public class GameAbility :IPoolObject, ITickable
    {
        ...
        internal void GF_ActivateAbilityWithGameEventParam(GameEventArg param)
        {
            if (param == null)
            {
                GameLogger.Log($"Activate ability with param failed, param is null. {AbilityName} of {Owner.UnitName}");
                return;
            }
            ...
            if (checkResult != ECheckAbilityResult.Success)
            {
                GameLogger.Log($"Activate ability with param failed, check result : {checkResult}. {AbilityName} of {Owner.UnitName}");
                return;
            }
            
            CommitAbility();
            GameLogger.Log($"Activate ability with param succeeded. {AbilityName} of {Owner.UnitName}");
